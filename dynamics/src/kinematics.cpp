...
State Kinematics::update(
            ...
            double dt)
{
    double thresh = 0.5;        // cut off to avoid singular behavior
    double err = 0.03;          // deadband to avoid flip flop

    if (start.velocity < thresh)
    {
        return update_low_vel(
                        start,
                        acc,
                        steer_angle_vel,
                        p,
    ...
    // for ease of the next two calculations
    double rear_vel = g * p.l_r - acc * p.cg_height;
    double front_vel = g * p.l_f + acc * p.cg_height;
...
State Kinematics::update_low_vel(
        const State start,
        double acc,
        double steer_angle_vel,
        CarParams p,
        double dt)
{
    ...
    double theta_double_dot = acc / p.wheelbase * std::tan(start.steer_angle) + 
